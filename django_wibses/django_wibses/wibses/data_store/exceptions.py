from . import HTTP__EXCEPTION_TAG__MISSING_PARAMS, HTTP__EXCEPTION_TAG__NOT_PROPER_REQUEST_TYPE, \
    ...
    HTTP__EXCEPTION_TAG__NOT_JSON_OBJECT, HTTP__EXCEPTION_TAG__SCRIPT_MANAGER_NOT_INITIALIZED
...
class NoSuchScriptInStorageException(Exception):
    def __init__(self, script_id):
        description = HTTP__EXCEPTION_TAG__NO_SUCH_SCRIPT_IN_STORAGE + ": %s" % script_id
...
class BadScriptRevisionException(Exception):
    def __init__(self, revision):
        description = HTTP__EXCEPTION_TAG__BAD_SCRIPT_REVISION + ": %s" % revision
...
class ScriptAlreadyExistsInStorageException(Exception):
    def __init__(self, script_name):
        description = HTTP__EXCEPTION_TAG__SCRIPT_ALREADY_IN_STORAGE + ": %s" % script_name
...
class RequestTypeDoesNotSupportedException(Exception):
    def __init__(self, request_type, supported_request_types):
        description = HTTP__EXCEPTION_TAG__NOT_SUPPORTED_REQUEST_TYPE + ": '%s' . Supported are: " % request_type
...
class NotSupportedApiActionException(Exception):
    def __init__(self, action, supported_actions):
        ...
        self._action = action
        description = HTTP__EXCEPTION_TAG__NOT_SUPPORTED_ACTION + ": '%s' . Supported are: " % action
...
class ScriptManagerNotInitializedException(Exception):
    def __init__(self):
        Exception.__init__(self, HTTP__EXCEPTION_TAG__SCRIPT_MANAGER_NOT_INITIALIZED)